  ...
  <div class="border rounded p-3 bg-light">
    ...
    <div v-if="!isEditing">
      <div class="d-flex justify-content-between align-items-center mb-2">
        ...
        <button class="btn btn-sm btn-outline-primary" @click="startAdd">
      ...
      <table class="table table-sm table-bordered bg-white mb-0">
        ...
        <tbody>
          <tr v-if="!params || params.length === 0">
            <td colspan="4" class="text-center text-muted small">{{ $t('tsl_no_data') }}</td>
          </tr>
          <tr v-for="(param, index) in params" :key="index">
            ...
            <td>
              <button class="btn btn-xs btn-link p-0 me-2" @click="startEdit(index)">{{ $t('tsl_edit') }}</button>
              <button class="btn btn-xs btn-link p-0 text-danger" @click="removeParam(index)">{{ $t('tsl_delete') }}</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <!-- Edit Form -->
    <div v-else class="bg-white p-3 border rounded">
      <h6 class="mb-3">{{ editingIndex === -1 ? $t('tsl_add') : $t('tsl_edit') }} {{ $t('tsl_param') }}</h6>
      <div class="row g-2">
        <div class="col-md-5">
      ...
      <div class="mt-3 text-end">
        <button class="btn btn-sm btn-secondary me-2" @click="cancelEdit">{{ $t('tsl_cancel') }}</button>
        <button class="btn btn-sm btn-primary" @click="saveParam">{{ $t('tsl_confirm') }}</button>
      </div>
    </div>
  </div>
</template>
