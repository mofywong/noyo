...
type ProtocolProfile struct {
	...
	// Stored as JSON string
	Config string `json:"config"`
	// IsSystem marks whether this profile is managed/generated by system
	IsSystem bool `gorm:"-" json:"is_system"`
}
...
func GetProtocolProfile(code string) (*ProtocolProfile, error) {
	...
	pp.IsSystem = isSystemProfileCode(pp.Code) || (pp.TenantID == 0 && pp.ProjectID == 0)
	return &pp, nil
}
...
func ListProtocolProfiles(page, pageSize int, tenantID, projectID uint, allowedProjectIDs ...[]uint) ([]ProtocolProfile, int64, error) {
	...
	if result.Error == nil {
		for i := range profiles {
			profiles[i].IsSystem = isSystemProfileCode(profiles[i].Code) || (profiles[i].TenantID == 0 && profiles[i].ProjectID == 0)
		}
	}
