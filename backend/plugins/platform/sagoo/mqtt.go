...
func (p *SagooPlugin) Start() error {
	cfg := p.Ctx.GetConfig()
	...
	// p.Logger.Info("Starting Sagoo Plugin",
...
func (p *SagooPlugin) messageSenderLoop() {
	defer p.wg.Done()
	for {
		select {
		case <-p.ctx.Done():
...
func (p *SagooPlugin) OnDisconnect() {
	// p.Logger.Info("Disconnected from Sagoo Platform")
}

func (p *SagooPlugin) OnConnect() {
	// p.Logger.Info("Connected to Sagoo Platform",
		// zap.String("broker", p.Config.Broker),
