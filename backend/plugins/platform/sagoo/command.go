...
func (p *SagooPlugin) subscribeToCommands() {
...
func (p *SagooPlugin) handleCommand(client mqtt.Client, msg mqtt.Message) {
	topic := msg.Topic()
	payload := msg.Payload()
	...
	if serviceId == "property/set" {
		...
		if !ok {
			...
			reply.Code = 400
			reply.Message = "Invalid params format"
		} else {
...
// Since SagooPlugin is Northbound (Gateway -> Platform), it receives commands via MQTT (Upstream from Device perspective, but Downlink from Platform).
